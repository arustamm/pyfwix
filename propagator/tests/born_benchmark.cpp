...
int main(int argc, char **argv) {
    ...
    std::vector<std::shared_ptr<complex4DReg>> dmodel; // Renamed 'slow_den' to 'model'
    ...
    dmodel.resize(2);
    dmodel[0] = std::make_shared<complex4DReg>(modelHyper);
    dmodel[1] = std::make_shared<complex4DReg>(modelHyper);
    dmodel[0]->set(1.0f); // Set model to constant 1.0
    dmodel[1]->set(1.0f); // Set model to constant 1.0

    std::cout << "Setting up StreamingPropagator..." << std::endl;
    // Use the parsed/default 'batches' vector from command line
    auto prop = std::make_shared<Propagator>(domainHyper, rangeHyper, slowHyper, wavelet,
        sx_all, sy_all, sz_all, s_ids_all,
        rx_all, ry_all, rz_all, r_ids_all,
        par); // Pass the vector here
    ...
    std::cout << "Setting up ExtendedBorn..." << std::endl;
    // Use the parsed/default 'batches' vector from command line
    ...
    // --- Calculate and Report CPU Duration ---
    duration_cpu_ms = std::chrono::duration_cast<std::chrono::milliseconds>(end_cpu_time - start_cpu_time);
    duration_cpu_s = std::chrono::duration_cast<std::chrono::duration<double>>(end_cpu_time - start_cpu_time); // For seconds with fraction
