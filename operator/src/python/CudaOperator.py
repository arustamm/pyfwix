...
class Spline4D (Op.Operator):
	def __init__(self,model: superVector, data: superVector, type='CR-spline',taper=[0,0,0]):
		if type == 'CR-spline':
			self.a = 0.5
			self.b = 0
		if type == 'B-spline':
		...
		if type == 'MN-spline':
			self.a = 1/3
			self.b = 1/3
		# first fill in nz-nfchunk-ny-nx
		nss = [model[0][0].getHyper().getAxis(i+1).n for i in range(4)]
		oss = [model[0][0].getHyper().getAxis(i+1).o for i in range(4)]
		dss = [model[0][0].getHyper().getAxis(i+1).d for i in range(4)]

		nfreq = sum([vec[0].getHyper().getAxis(3).n for vec in model.vecs])
		nss[3] = nfreq
		print("Spline4D nss:", nss)
		print("Spline4D dss:", dss)
		print("Spline4D oss:", oss)

		self.mod4d = SepVector.getSepVector(ns=nss, os=oss, ds=dss, storage='dataComplex')
		self.dat4d = SepVector.getSepVector(ns=nss, os=oss, ds=dss, storage='dataComplex')


		self.taper = taper
		self.cppMode = pyCudaOperator.Spline4D(self.mod4d.cppMode,self.dat4d.cppMode,self.a,self.b,taper)
		self.setDomainRange(self.mod4d, self.dat4d)
